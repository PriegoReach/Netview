...
import { RatingEditPage } from '../rating-edit/rating-edit.page';
import axios from 'axios';
import { environment } from 'src/environments/environment';

@Component({
  selector: 'app-rating-detail',
  templateUrl: './rating-detail.page.html',
  styleUrls: ['./rating-detail.page.scss'],
  standalone: false,
})
export class RatingDetailPage {
  ...
  async cargarDetalle() {
    const id = this.route.snapshot.paramMap.get('rating_id');
    const loading = await this.loadingCtrl.create({ message: 'Cargando', spinner: 'bubbles' });
    await loading.present();
    await axios({
      method: 'get',
      url: environment.apiUrl + '/rating/' + id,
  ...
  async eliminar(id: any) {
    await axios({
      ...
      const alert = await this.alertCtrl.create({
        ...
        buttons: [{ text: 'Continuar', handler: () => { this.router.navigate(['/rating-list']); } }]
      });
      await alert.present();
    }).catch(async (error) => {
