...
export class SubscriptionDetailPage {
  ...
  async cargarDetalle() {
    const id = this.route.snapshot.paramMap.get('subscription_id');
    const loading = await this.loadingCtrl.create({ message: 'Cargando', spinner: 'bubbles' });
    await loading.present();
    await axios({
      method: 'get',
      url: environment.apiUrl + '/subscription/' + id,
  ...
  async editar(id: any) {
    const modal = await this.modalCtrl.create({
      ...
      componentProps: { 'subscription_id': id },
  ...
  async eliminar(id: any) {
    await axios({
      ...
      const alert = await this.alertCtrl.create({
        ...
        buttons: [{ text: 'Continuar', handler: () => { this.router.navigate(['/subscription-list']); } }]
      });
      await alert.present();
    }).catch(async (error) => {
