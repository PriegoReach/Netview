...
import { LanguageEditPage } from '../language-edit/language-edit.page';
...
export class LanguageDetailPage {
  ...
  async cargarDetalle() {
    const id = this.route.snapshot.paramMap.get('language_id');
    const loading = await this.loadingCtrl.create({ message: 'Cargando', spinner: 'bubbles' });
    await loading.present();
    await axios({
      method: 'get',
      url: environment.apiUrl + '/language/' + id,
  ...
  async eliminar(id: any) {
    await axios({
      ...
      const alert = await this.alertCtrl.create({
        ...
        buttons: [{ text: 'Continuar', handler: () => { this.router.navigate(['/language-list']); } }]
      });
      await alert.present();
    }).catch(async (error) => {
